...
import { type Block, type Blocks, BlocksSchema } from "../contracts/tank/blocks/v1/blocks_pb.js";
...
function normalizeElement(e: RichTextElement): RichTextElement {
  const k = e.kind;
  switch (k.case) {
    case "text":
      return {
...
export function normalizeRichText(r: RichText | undefined): RichText | undefined {
  if (!r) return r;
  return {
    ...r,
...
function normalizeKind(kind: Block["kind"]): Block["kind"] {
  switch (kind.case) {
    case "header":
...
function normalizeButton<B extends { text: string; value: string; url: string; actionId: string }>(b: B): B {
  return {
    ...b,
 ...
 */
export function normalizeBlocks(input: Blocks | Block[]): Blocks {
  ...
  for (let i = 0; i < capped.length; i++) {
    const b = capped[i]!;
    if (b.kind.case === undefined) continue;
    let blockId = b.blockId.trim() || `b${i + 1}`;
    if (seen.has(blockId)) {
      let n = 2;
      while (seen.has(`${blockId}_${n}`)) n++;
      blockId = `${blockId}_${n}`;
    }
    seen.add(blockId);
...
export function validateBlocks(input: Blocks | Block[]): BlockIssue[] {
  const list = Array.isArray(input) ? input : input.blocks;
  ...
  list.forEach((b, i) => {
    ...
    if (b.blockId) {
      ...
      if (prev !== undefined) issues.push({ path: `${p}.block_id`, message: `duplicate of blocks[${prev}]` });
      ids.set(b.blockId, i);
    }
    const k = b.kind;
    switch (k.case) {
      case undefined:
        ...
        break;
      case "header":
        if (!k.value.text.trim()) issues.push({ path: `${p}.header.text`, message: "empty" });
        break;
      case "section":
        if (!k.value.text && k.value.fields.length === 0)
          issues.push({ path: `${p}.section`, message: "needs text or fields" });
        if (k.value.accessory) checkButton(k.value.accessory, `${p}.section.accessory`, issues);
        break;
      case "context":
        if (k.value.elements.length === 0) issues.push({ path: `${p}.context.elements`, message: "empty" });
        break;
      case "actions":
        if (k.value.buttons.length === 0) issues.push({ path: `${p}.actions.buttons`, message: "empty" });
        k.value.buttons.forEach((btn, j) => {
          checkButton(btn, `${p}.actions.buttons[${j}]`, issues);
        });
        break;
      case "planCard":
        if (!k.value.summary.trim()) issues.push({ path: `${p}.plan_card.summary`, message: "empty" });
        k.value.steps.forEach((s, j) => {
          if (!s.id) issues.push({ path: `${p}.plan_card.steps[${j}].id`, message: "empty" });
        });
        break;
      case "diffPreview":
        if (!k.value.commitSha) issues.push({ path: `${p}.diff_preview.commit_sha`, message: "empty" });
        break;
      case "ciStatus":
        if (!k.value.headSha) issues.push({ path: `${p}.ci_status.head_sha`, message: "empty" });
        break;
      case "approvalPrompt":
        if (!k.value.gateId) issues.push({ path: `${p}.approval_prompt.gate_id`, message: "empty" });
        if (!k.value.subject.trim()) issues.push({ path: `${p}.approval_prompt.subject`, message: "empty" });
        break;
      case "toolLog":
        if (!k.value.phase.trim()) issues.push({ path: `${p}.tool_log.phase`, message: "empty" });
        break;
      case "filePreview":
        if (!k.value.fileId) issues.push({ path: `${p}.file_preview.file_id`, message: "empty" });
        break;
      case "statusCard":
        if (!k.value.runId) issues.push({ path: `${p}.status_card.run_id`, message: "empty" });
        break;
      default:
        break;
    }
...
function checkButton(
  b: { actionId: string; url: string; text: string },
  path: string,
  issues: BlockIssue[],
): void {
  if (!b.text.trim()) issues.push({ path: `${path}.text`, message: "empty" });
  if (!b.actionId && !b.url) issues.push({ path: `${path}.action_id`, message: "needs action_id or url" });
}
