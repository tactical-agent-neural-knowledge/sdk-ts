...
import { Lane, MarkSchema, MarkType } from "../contracts/tank/topo/v1/topo_pb.js";
 ...
 */
export function marksFromSearchHits(hits, channelId) {
    const marks = [];
    for (const hit of hits) {
        const msg = hit.message;
        // A workspace-wide search can return other channels; a strip only maps its own.
        if (!msg || hit.channelId !== channelId)
            continue;
        marks.push(create(MarkSchema, {
            id: `search:${msg.id}`,
            channelId,
            messageId: msg.id,
