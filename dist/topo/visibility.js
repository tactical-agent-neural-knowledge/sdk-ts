 ...
 * anything.
 */
export const DEFAULT_VISIBLE_MARKS = [
    MarkType.MENTION,
    MarkType.WAITING_ON,
 ...
 */
export function isAlwaysVisible(type) {
 ...
 */
export function visibleMarkTypes(prefs) {
    if (!prefs?.configured)
        return new Set(DEFAULT_VISIBLE_MARKS);
    return new Set(prefs.visible);
}
/** Whether a mark should be drawn, given the person's preferences. */
export function isMarkVisible(type, prefs) {
